...
def play_game(starting_numbers, iterations=2020, verbose=True):
    ...
    if verbose:
        print(
            f"this_turn:{this_turn_number}, this_number: {this_number}, dict: {value_seen_at}"
        )
    ...
    while this_turn_number < iterations:
        ...
        if len(value_seen_at[this_number]) > 2:
            value_seen_at[this_number] = value_seen_at[this_number][-2:]

        # and print if we're printing
        if verbose:
            if this_turn_number % 100000 == 0:
                print(f"this_turn:{this_turn_number}, this_number: {this_number}")
...
# part2, mo' iterations, mo' problems..
part2_iteration_count = 30000000
part2_answer = play_game(puzzle_input, iterations=part2_iteration_count, verbose=True)
