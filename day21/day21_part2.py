...
class FoodDB:
    ...
    def part2_answer(self):
        """
        Alphabetical list, comma separated, no spaces, of the ingredients with allergens in them
        """
        # APLHABETICALLY BY THEIR ALLERGEN!!
        sorted_allergens = sorted(self.known_allergens.keys())
        sorted_danger = []
        for this_allergen in sorted_allergens:
            sorted_danger.append(self.known_allergens[this_allergen])
        result = ",".join(sorted_danger)
