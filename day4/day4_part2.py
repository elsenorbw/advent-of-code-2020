...
def range_validator(value_to_check, params):
    """
    value_to_check: string representation of a number
    params: tuple of lower and upper acceptable range values
    """
    min_acceptable_value, max_acceptable_value = params
    value_to_check = int(value_to_check)
    result = (
        value_to_check >= min_acceptable_value
        and value_to_check <= max_acceptable_value
    )

    #    print(
    #        f"range_validator: {value_to_check} between {min_acceptable_value} and {max_acceptable_value} -> {result} "
    #    )
    return result


def height_validator(value_to_check, params):
    """
    hgt (Height) - a number followed by either cm or in:
        If cm, the number must be at least 150 and at most 193.
        If in, the number must be at least 59 and at most 76.
    ...
    # ok, it needs to end cm, in otherwise it's bobbins
    units = value_to_check[-2:]
    value_to_check = value_to_check[:-2]

    if "cm" == units:
        result = range_validator(value_to_check, (150, 193))
    elif "in" == units:
        result = range_validator(value_to_check, (59, 76))
    else:
        print(f"What the hell type of units are {units} then ?")
        result = False

    print(f"height_validator: {value_to_check} ({units}) -> {result}")
    return result


def character_validator(value_to_check, params):
    """
    params : (expected length, acceptable characters)
    ensure that value_to_check is a string of exactly expected length and contains only accpetable characters
    """
    result = True
    expected_length, acceptable_characters = params

    if len(value_to_check) != expected_length:
        result = False
    else:
        matches = [this_char in acceptable_characters for this_char in value_to_check]
        print(f"matches: {matches}")
        result = all(matches)

    print(
        f"char_validation: val=[{value_to_check}], len={expected_length}, acceptable={acceptable_characters} -> {result}"
    )
    return result


def colour_validator(value_to_check, params):
    """
    hcl (Hair Color) - a # followed by exactly six characters 0-9 or a-f.
    """
    result = True
    ...
    if not value_to_check.startswith("#"):
        result = False
    else:
        result = character_validator(value_to_check[1:], (6, "0123456789abcdef"))

    print(f"colour_validator: {value_to_check} -> {result}")
    return result


def list_validator(value_to_check, params):
    """
    value_to_check must be in the list provided as params
    """
    result = value_to_check in params
    print(f"list_validator: {value_to_check} valid={params} -> {result}")
    return result
...
class Passport:
    ...
    def is_valid(self) -> bool:
        ...
        if it does then make sure each field matches the required data validation rules:
            byr (Birth Year) - four digits; at least 1920 and at most 2002.
            iyr (Issue Year) - four digits; at least 2010 and at most 2020.
            eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
            hgt (Height) - a number followed by either cm or in:
        ...
        field_validation = {
            ...
            "hgt": (height_validator, None),
            "hcl": (colour_validator, None),
            "ecl": (list_validator, ("amb", "blu", "brn", "gry", "grn", "hzl", "oth")),
            "pid": (character_validator, (9, "0123456789")),
        }
        result = True
        for this_fieldname in field_validation:
            # do we have the field at all ?
            if this_fieldname not in self.fields:
                result = False
            else:
                # get the validator function and params
                validator, validation_params = field_validation[this_fieldname]
                the_value = self.fields[this_fieldname]
                if validator is not None:
                    if not validator(the_value, validation_params):
                        result = False
