...
class Deck:
    ...
    def tuple(self):
        """
        Return a tuple as a hash value
        """
    ...
    def print(self):
        print(f"Deck {self.name}({self.level}): {self.cards}")
    ...
    def subdeck(self, number_of_cards: int):
        """
        duplicate up this deck but only the top x cards..
        """
        new_deck = Deck(self.name, self.level + 1)
        new_deck.cards = self.cards[:number_of_cards]
        return new_deck
...
def play_combat(deck1, deck2):
    ...
    while deck1.has_cards() and deck2.has_cards():
        ...
        configuration_key = (deck1.tuple(), deck2.tuple())
        if configuration_key in previous_configurations:
            # print(f"we've seen this configuration before, player 1 instant win!")
            return True
        else:
            previous_configurations.add(configuration_key)

        # get the top cards and see who wins..
        card1 = deck1.deal()
        card2 = deck2.deal()

        # can we do recursive battle ?
        if deck1.count() >= card1 and deck2.count() >= card2:
            subdeck1 = deck1.subdeck(card1)
            subdeck2 = deck2.subdeck(card2)
            first_player_wins_round = play_combat(subdeck1, subdeck2)
        else:
            if card1 > card2:
                first_player_wins_round = True
            else:
                first_player_wins_round = False

        if first_player_wins_round:
    ...
    if deck1.has_cards():
        score = deck1.score()
        print(f"Winning p1 score is {score}")
    else:
        score = deck2.score()
        print(f"Winning p2 score is {score}")

    return deck1.has_cards()
